...
export default async function AssetsPage() {
  ...
  totalGain = assets.reduce((sum, asset) => sum + (Number(asset.current_value) - Number(asset.purchase_value)), 0)

  const gainPercentage = assets.reduce((sum, asset) => {
    const purchaseValue = Number(asset.purchase_value)
    if (purchaseValue === 0) return sum
    return sum + ((Number(asset.current_value) - purchaseValue) / purchaseValue) * 100
  }, 0)

  const avgGainPercentage = assets.length > 0 ? gainPercentage / assets.length : 0
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Track your investments and wealth</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div
              className={`flex items-center gap-2 text-2xl font-bold ${totalGain >= 0 ? "text-green-600" : "text-red-600"}`}
            >
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${avgGainPercentage >= 0 ? "text-green-600" : "text-red-600"}`}>
              {avgGainPercentage >= 0 ? "+" : ""}
