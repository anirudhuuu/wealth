...
export function AssetsList({ assets, isAdmin }: AssetsListProps) {
  ...
  const getAssetTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      ...
      mutual_fund: "bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-300",
      stock: "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300",
      gold: "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300",
      real_estate: "bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-300",
  ...
  return (
    <>
          ...
          <div className="space-y-3">
            {assets.length === 0 ? (
              <div className="py-8 text-center text-sm text-muted-foreground">
                ...
                <p>No assets yet</p>
                {isAdmin && <p className="mt-1">Add your first asset to start tracking your wealth</p>}
              </div>
            ) : (
              assets.map((asset) => {
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${getAssetTypeColor(asset.type)}`}
                          >
                        ...
                        <div className="mt-2 grid gap-2 text-sm md:grid-cols-2">
                          <div>
                            ...
                            <span className="ml-2 font-semibold">{formatCurrency(currentValue)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Purchase Value:</span>
                            <span className="ml-2 font-medium">{formatCurrency(purchaseValue)}</span>
                          </div>
                          <div>
                          ...
                          {asset.maturity_date && (
                            <div>
                              ...
                              <span className="ml-2">{formatDate(asset.maturity_date)}</span>
                            </div>
                          )}
                        </div>

                        {asset.notes && <p className="mt-2 text-sm text-muted-foreground">{asset.notes}</p>}
                      </div>

                      <div className="ml-4 text-right">
                        <div
                          className={`flex items-center gap-1 text-lg font-bold ${gain >= 0 ? "text-green-600" : "text-red-600"}`}
                        >
      ...
      {isAdmin && <AddAssetDialog open={showAddDialog} onOpenChange={setShowAddDialog} />}
    </>
  )
}
